...
- Codes written by Jemba developers should be pulled from Jemba Repo into the Jenkins server and sent them to the EC2's from here with Ansible.
...
In the architecture, you can configure your architecture with these conditions,
  ...
  - `Devops_files` file should be pulled from Github Repo at the beginning. Then these files should be copy to related folders.
...
```bash
101:Jemba_jenkins_project (folder)
|
...
|----Devops_files (folder)     # Given to the Devops Team (Postgresql, Nodejs and React files)
